...
public class ContextAssembler {
    ...
    public static List<LlmMessage> assemble(SessionContext sessionContext, boolean forLog) {
        ...
        List<ConversationTurn> recentTurns =
                sessionContext.getTurns().subList(fromIndex, sessionContext.getTurns().size());
        ...
        for (ConversationTurn turn : recentTurns) {
            ...
            if (turn.isSupersededBeforeAssistantReply()) {
                if (forLog) {
                    messages.add(new LlmMessage("system", "PREVIOUS_USER_MESSAGE_SUPERSEDED"));
                } else {
                    messages.add(
                            new LlmMessage(
                                    "system",
            ...
            if (turn.isAssistantPlaybackInterrupted()) {
                if (forLog) {
                    messages.add(new LlmMessage("system", "ASSISTANT_PLAYBACK_INTERRUPTED"));
                } else {
                    messages.add(
                            new LlmMessage(
                                    "system",
    ...
    public List<LlmMessage> assemble(
            SessionContext sessionContext, String userText, boolean forLog) {
        List<LlmMessage> messages = new ArrayList<>();
